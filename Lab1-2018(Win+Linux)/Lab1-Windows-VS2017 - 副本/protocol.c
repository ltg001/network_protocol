...
static struct option intopts[] = {
	...
	{ "data_timer", required_argument, NULL, 'D'},
	{ "ack_timer", required_argument, NULL, 'A'},
	{ "window_number", required_argument, NULL, 'W'},
	{ 0, 0, 0, 0 },
};

#define OPT_SHORT "?ufind:p:b:l:t:D:A:W:"
...
static void config(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argv, OPT_SHORT, intopts, NULL)) != -1) {
		...
		case 'D':
			DATA_TIMER = atoi(optarg);
			printf("get data_timer: %d\n", atoi(optarg));
			break;

		case 'A':
			ACK_TIMER = atoi(optarg);
			printf("get ack_timer: %d\n", atoi(optarg));
			break;

		case 'W':
			WINDOW_NUMBER = atoi(optarg);
			printf("get window_number: %d\n", atoi(optarg));
			break;
